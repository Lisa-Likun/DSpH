...
from proxy_loss.loss_sui import Norm_SoftMax
import os
...
class Trainer(TrainBase):
    ...
    def _init_model(self):
        ...
        HashModel = DCMHT
        ##############################################
        # self.criterion = Norm_SoftMax(self.args).cuda()
        _embeddings = torch.randn(self.args.batch_size, self.args.output_dim).cuda()
        _labels = torch.tensor([[1, 0, 0], [0, 1, 0], [0, 0, 1]]).float().cuda()
        _labels = F.pad(_labels, (0, self.args.n_classes - 3, 0, 0), "constant", 0)
        _targets = torch.argmax(_labels, dim=1).int()

        args = Namespace(n_bits=self.args.output_dim, n_classes=self.args.n_classes, scale=23.0, ps_alpha=0.40, ps_mu=0.8, method=1)

        self.criterion = Norm_SoftMax(args).cuda()
        ##############################################
    ...
    def _init_dataset(self):
        ...
        self.args.index_file = os.path.join("/root/autodl-tmp/DCHMT-main/dataset", self.args.dataset, self.args.index_file)
        self.args.caption_file = os.path.join("/root/autodl-tmp/DCHMT-main/dataset", self.args.dataset, self.args.caption_file)
        self.args.label_file = os.path.join("/root/autodl-tmp/DCHMT-main/dataset", self.args.dataset, self.args.label_file)
    ...
    def train_epoch(self, epoch):
        ...
        for image, text, label, index in self.train_loader:
            ...
            # criterion = Proxy_NCA(args.dim, args.C, scale=args.scale,
            ...
            loss_image = self.criterion(hash_img, label)
            loss_text = self.criterion(hash_text, label)
            # print(type(hash_img))
            # exit()
            # loss_quantization_image = np.log(np.cosh(torch.sub(hash_img, 1.0)))
            # loss_quantization_text = np.log(np.cosh(torch.sub(hash_text, 1.0)))
            # loss_quantization_image = torch.log(torch.cosh(torch.sub(hash_img, 1.0)))
            # loss_quantization_text = torch.log(torch.cosh(torch.sub(hash_text, 1.0)))

            loss_quantization_image = torch.sub(hash_img, torch.sign(hash_img))
            loss_quantization_text = torch.sub(hash_text, torch.sign(hash_text))

            loss_quantization_image = torch.abs(loss_quantization_image)
            loss_quantization_text = torch.abs(loss_quantization_text)

            loss_quantization_image = torch.sum(loss_quantization_image,dim=0)
            loss_quantization_text = torch.sum(loss_quantization_text,dim=0)

            loss_quantization_image = torch.sum(loss_quantization_image)
            loss_quantization_text = torch.sum(loss_quantization_text)
            # print(loss_quantization_image)
            # print(loss_quantization_text)
            # exit()
            ...
            loss = loss_image + loss_text + loss_quantization_image + loss_quantization_text
